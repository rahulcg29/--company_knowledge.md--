...
class CGBankDatabase:
    ...
    def _save_data():
        ...
        try:
            ...
            with open('dummydata.json', 'w') as f:
                json.dump(BANK_DATA, f, indent=2, default=datetime_serializer)
            return True
        except Exception as e:
    ...
    def create_user(username: str, password: str, user_data: Dict[str, Any]) -> bool:
        ...
        # Initialize empty transaction history for the new user
        if 'user_transactions' not in BANK_DATA:
            BANK_DATA['user_transactions'] = {}
        BANK_DATA['user_transactions'][username.lower()] = []
    ...
    def get_user_transactions(username: str) -> List[Dict[str, Any]]:
        ...
        # Check if user has their own transaction history
        if 'user_transactions' in BANK_DATA and username.lower() in BANK_DATA['user_transactions']:
            # Convert string dates back to datetime objects
            transactions = BANK_DATA['user_transactions'][username.lower()]
            for txn in transactions:
                if isinstance(txn['date'], str):
                    txn['date'] = datetime.strptime(txn['date'], '%Y-%m-%d %H:%M:%S')
            return transactions
        
        # For new users, generate some initial transactions
        transactions = []
        for i in range(3):
            transaction = {
        ...
        if 'user_transactions' not in BANK_DATA:
            BANK_DATA['user_transactions'] = {}
        BANK_DATA['user_transactions'][username.lower()] = transactions
    ...
    def add_transaction(username: str, description: str, amount: float) -> bool:
        ...
        # Add to user's transaction history in BANK_DATA
        if 'user_transactions' not in BANK_DATA:
            BANK_DATA['user_transactions'] = {}
        if username.lower() not in BANK_DATA['user_transactions']:
            BANK_DATA['user_transactions'][username.lower()] = []
        BANK_DATA['user_transactions'][username.lower()].insert(0, new_transaction)
...
class CGBankApp:
    ...
    def _render_create_account_form(self):
        ...
        with st.form("create_account_form"):
            ...
            aadhar_number = st.text_input("Aadhar Number", placeholder="Eg:(1234-5678-9012)")
            pan_number = st.text_input("PAN Number", placeholder="Eg:(HJUPP9663L)").upper()
